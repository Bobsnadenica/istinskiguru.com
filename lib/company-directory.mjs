...
function validateCheck(check, review) {
  if (!check.name || !check.title || !check.finding || !check.assessment || !statuses[check.status] || !/^\d{4}-\d{2}-\d{2}$/.test(check.reviewedOn) || !check.sources?.length || !Array.isArray(check.companies)) throw new Error('Incomplete profile company check');
