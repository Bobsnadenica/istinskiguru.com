import { renderCompanyDirectory } from "./company-directory.mjs";
// One escaped HTML representation for the static profile and gallery dialog.
export function renderReview(review) {
  if (review.kind === "connections") return renderCompanyDirectory(review);
  const text = (value) => {
  ...
  const list = (items) => {
    if (!Array.isArray(items) || !items.length) throw new Error("Review requires scope");
  ...
  return `<article class="evidence-review" aria-label="Проверка по публични източници">
    <header class="evidence-verdict"><p class="eyebrow">Публични източници към <time datetime="${text(review.reviewedOn)}">${text(review.reviewedOn.split("-").reverse().join("."))}</time></p>
    ...
    <p class="source-note"><a href="/contactus.html">Изпрати документ, корекция или право на отговор.</a></p>
  </article>`;
}
