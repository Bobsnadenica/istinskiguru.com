...
<html lang="bg">
  ...
  <body class="profile-page${profile.reviewHtml ? " evidence-profile" : ""}${profile.reviewKind === "connections" ? " connections-profile" : ""}">
    ...
    <div class="page-shell">
      <header class="hero article-hero profile-share-hero" id="top">
        <div class="hero-copy">
          <nav class="page-nav" aria-label="Навигация">
            <a href="${profile.reviewHtml ? "/#directory" : "/gallery.html#directory"}">← ${profile.reviewHtml ? "Всички разследвания" : "Архивни профили"}</a>
          </nav>
          <p class="eyebrow">${profile.reviewKind === "connections" ? "Фирмен справочник • Публични източници" : profile.reviewHtml ? "Разследване • Публични източници" : "Архив • Редакционен коментар"}</p>
          <h1>${escapeHtml(profile.name)}</h1>
          <p class="hero-text">${escapeHtml(profile.summary || profile.description || profile.imageNote)}</p>
          <p class="context-note">
            ${profile.reviewKind === "connections" ? "Собственост, управление и делови роли — с дата и източник." : profile.reviewHtml ? "Оферта, обещания и условия — проверени по посочените източници." : "Редакционен коментар с елементи на сатира. Присъствието в регистъра не означава установена измама или участие в Понци схема. Публичните канали по-долу не са независимо потвърждение на твърденията."} <a href="/contactus.html">Изпрати корекция или източник.</a>
          </p>
      ...
      <main class="profile-share-main" id="main">
        ${profile.reviewHtml ? `<div class="evidence-page">${profile.reviewHtml}<div class="share-button-group" aria-label="Опции за споделяне">${shareButtons.join("")}</div></div>` : `<article class="profile-card" id="${escapeHtml(profile.id)}" data-orientation="${escapeHtml(profile.orientation)}">
