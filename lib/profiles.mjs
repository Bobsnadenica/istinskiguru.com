...
<html lang="bg">
  ...
  <body class="profile-page${profile.reviewHtml ? " evidence-profile" : ""}">
    ...
    <div class="page-shell">
      <header class="hero article-hero profile-share-hero" id="top">
        <div class="hero-copy">
          ...
          <p class="eyebrow">${profile.reviewHtml ? "Разследване • Публични източници" : "Архив • Редакционен коментар"}</p>
          <h1>${escapeHtml(profile.name)}</h1>
          <p class="hero-text">${escapeHtml(profile.summary || profile.description || profile.imageNote)}</p>
          <p class="context-note">
            ${profile.reviewHtml ? "Оферта, обещания и условия — проверени по посочените източници." : "Редакционен коментар с елементи на сатира. Присъствието в регистъра не означава установена измама или участие в Понци схема. Публичните канали по-долу не са независимо потвърждение на твърденията."} <a href="/contactus.html">Изпрати корекция или източник.</a>
          </p>
...
export async function readProfiles(rootDir = process.cwd()) {
  ...
  for (const folder of folders) {
    ...
    if (files.some((entry) => entry.name === "review.json")) {
      ...
      reviewFields = { reviewHtml: renderReview(review), reviewedOn: review.reviewedOn, subjectType: review.subjectType,
