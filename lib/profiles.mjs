import { siteNav, siteFooter, followPanel, writeDirectories } from "./site.mjs";
import { renderReview } from "./review.mjs";
import { renderProfileCompanyCheck } from "./company-directory.mjs";
...
const landscapeImageNotes = [
  ...
  "Широкоформатно присъствие, подготвено да побере обещание, авторитет и плавно следващо плащане.",
...
const siteShareCardFileName = "registry-share-v2.jpg";
const optimizedImageMaxDimension = 1600;
const optimizedJpegQuality = "72";
const thumbnailImageMaxDimension = 520;
const thumbnailJpegQuality = "64";
const staticPageEntries = [
  { pathname: "", changefreq: "daily", priority: "1.0" },
  { pathname: "gallery.html", changefreq: "weekly", priority: "0.9" },
  { pathname: "contactus.html", changefreq: "monthly", priority: "0.75" },
  { pathname: "scams.html", changefreq: "weekly", priority: "0.85" },
  { pathname: "saveti.html", changefreq: "weekly", priority: "0.85" },
  { pathname: "privacy.html", changefreq: "monthly", priority: "0.5" },
  { pathname: "terms.html", changefreq: "monthly", priority: "0.5" },
  { pathname: "supportus.html", changefreq: "monthly", priority: "0.7" },
];
...
function getShareMessage(profile) {
  return normaliseText(profile.summary || profile.imageNote || `Профил на ${profile.name} в Регистъра на Онлайн Гурута.`);
}
...
function getProfileMetaDescription(profile) {
  if (profile.reviewedOn && profile.description) {
    return truncateText(normaliseText(profile.description), 240);
  }
  return truncateText(
    [
      `Профил на ${profile.name} в регистър за публични онлайн гуру обещания и анализ на чудесата.`,
      profile.summary || profile.imageNote || `Профил на ${profile.name} в регистъра на онлайн гурута.`,
      profile.insight || profile.kicker || profile.aura,
      "Виж публичните линкове, описанието и полевите бележки в Регистъра на Онлайн Гурута.",
    ]
      .filter(Boolean)
...
function getProfileKeywords(profile) {
  const keywords = [
    normaliseText(profile.name),
    `${normaliseText(profile.name)} курс`,
    `${normaliseText(profile.name)} ментор`,
    "онлайн гуру",
    "бизнес коуч",
    "ментор",
    "онлайн курс",
...
function buildSitemapXml(profiles, siteBaseUrl, buildDate = new Date()) {
  const lastmod = buildDate.toISOString().slice(0, 10);
  const urls = [
    ...staticPageEntries.map((entry) => ({
      loc: new URL(entry.pathname || "/", siteBaseUrl).toString(),
      changefreq: entry.changefreq,
      priority: entry.priority,
      lastmod,
    })),
    ...profiles.map((profile) => ({
      loc: getProfilePageUrl(profile, siteBaseUrl),
      changefreq: "weekly",
      priority: "0.8",
      lastmod,
    })),
...
function buildRobotsTxt(siteBaseUrl) {
...
async function generateShareCard(profile, sourceImagePath, outputImagePath) {
  if (await isOutputFresh(sourceImagePath, outputImagePath) &&
      (!(profile.guide || profile.shareTitle) || await isOutputFresh(path.join(process.cwd(), "lib/profiles.mjs"), outputImagePath))) {
    return;
  }
  ...
  const payload = {
    ...
    brand: "Регистър на Онлайн Гурута",
    name: profile.shareTitle ? profile.shareTitle.trim() : normaliseText(profile.name),
    kicker: truncateText(profile.kicker || profile.aura || "За какво да внимаваме", 72),
    summary: truncateText(profile.summary || profile.imageNote || profile.insight || "", 180),
    guide: Boolean(profile.guide || profile.shareTitle),
    logo: Boolean(profile.shareLogo),
  };
...
async function isOutputFresh(sourcePath, outputPath) {
  try {
    ...
    try {
      ...
      return outputStats.mtimeMs >= sourceStats.mtimeMs - 1000;
    } catch {
      return true;
    }
  } catch {
...
async function optimiseImageAsset(sourceImagePath, targetImagePath, options = {}) {
  if (await isOutputFresh(sourceImagePath, targetImagePath)) {
    return;
  }

  const { maxDimension = optimizedImageMaxDimension, jpegQuality = optimizedJpegQuality } = options;
  const extension = path.extname(targetImagePath).toLowerCase();
  const sipsArgs = [];

  if (extension === ".jpg" || extension === ".jpeg") {
    sipsArgs.push("-s", "format", "jpeg", "-s", "formatOptions", jpegQuality);
  }

  sipsArgs.push("-Z", String(maxDimension), sourceImagePath, "--out", targetImagePath);
...
async function copyAssetIfNeeded(sourcePath, targetPath) {
  if (await isOutputFresh(sourcePath, targetPath)) {
    return;
  }
...
function buildProfileSharePage(profile, siteBaseUrl) {
  ...
  const pageTitle = `${shareTitle} | Регистър на Онлайн Гурута`;
  const pageDescription = getProfileMetaDescription(profile);
  const pageKeywords = getProfileKeywords(profile);
  const imagePath = toRootRelativePath(profile.image);
  const thumbnailPath = toRootRelativePath(profile.thumbnailImage || profile.image);
  ...
  const shareImageAlt = profile.shareTitle
    ? `Редакционна карта: ${profile.name} — ${normaliseText(profile.shareTitle)}`
    : (profile.alt || profile.name);
  const profileImageUrl = new URL(imagePath, siteBaseUrl).toString();
  ...
  const siteRootUrl = new URL(siteBaseUrl).toString();
  const firstVideo = Array.isArray(profile.videos) && profile.videos.length ? toRootRelativePath(profile.videos[0]) : "";
  const sameAsLinks = (profile.links || []).map((link) => link.url).filter(Boolean);
  const structuredGraph = [
    {
      "@type": profile.reviewHtml ? "Article" : "CollectionPage",
      url: pageUrl,
      ...(profile.reviewedOn ? { dateModified: profile.reviewedOn } : {}),
      name: shareTitle,
      inLanguage: "bg-BG",
      ...
      isPartOf: {
        "@type": "WebSite",
        name: "Регистър на Онлайн Гурута",
        url: siteRootUrl,
      },
      about: {
        "@id": `${pageUrl}#${profile.subjectType === "Organization" ? "organization" : "person"}`,
      },
      mainEntity: {
        "@id": `${pageUrl}#${profile.subjectType === "Organization" ? "organization" : "person"}`,
      },
    ...
    {
      "@type": profile.subjectType || "Person",
      "@id": `${pageUrl}#${profile.subjectType === "Organization" ? "organization" : "person"}`,
      name: profile.name,
      description: pageDescription,
  ...
  if (profile.reviewKind === "connections") {
    structuredGraph[0]["@type"] = "CollectionPage";
    delete structuredGraph[0].about;
    delete structuredGraph[0].mainEntity;
    structuredGraph.splice(1, 1);
  }

  if (firstVideo) {
    structuredGraph.push({
      "@type": "VideoObject",
      name: `${profile.name} видео`,
      description: pageDescription,
      thumbnailUrl: shareImageUrl,
      contentUrl: new URL(firstVideo, siteBaseUrl).toString(),
      embedUrl: pageUrl,
      inLanguage: "bg-BG",
    });
  }
                ...
                (videoPath, index) => `
                  <div class="profile-video-frame">
                    <button
                      ...
                      type="button"
                      data-video-launch
                      data-video-src="${escapeHtml(toRootRelativePath(videoPath))}"
                      data-video-type="${escapeHtml(getVideoMimeType(videoPath))}"
                      data-video-poster="${escapeHtml(thumbnailPath)}"
                    ...
                    >
                      <img class="profile-video-poster" src="${escapeHtml(thumbnailPath)}" alt="" loading="lazy" decoding="async" fetchpriority="low" />
                      <span class="profile-video-overlay">
                        <span class="profile-video-badge">${escapeHtml(profileVideos.length > 1 ? `Видео ${index + 1}` : "Видео")}</span>
  ...
  const reportHelpLink = `<a class="button button-secondary" href="/saveti.html#help">При проблем с оферта</a>`;
...
<html lang="bg">
  <head>
    ...
    <link rel="alternate" hreflang="bg-BG" href="${escapeHtml(pageUrl)}" />
    <meta property="og:locale" content="bg_BG" />
    <meta property="og:type" content="article" />
    <meta property="og:site_name" content="Регистър на Онлайн Гурута" />
    <meta property="og:title" content="${escapeHtml(shareTitle)}" />
    <meta property="og:description" content="${escapeHtml(pageDescription)}" />
    <meta property="og:url" content="${escapeHtml(pageUrl)}" />
    <meta property="og:image" content="${escapeHtml(shareImageUrl)}" />
    <meta property="og:image:url" content="${escapeHtml(shareImageUrl)}" />
    ...
    <script type="application/ld+json">${serialiseJsonLd(structuredData)}</script>
    <link rel="icon" type="image/jpeg" sizes="96x96" href="/icon-96.jpg" />
    <link rel="stylesheet" href="/styles.css${profile.reviewHtml ? "?v=20260925-evidence-images" : ""}" />
    <link rel="stylesheet" href="/site.css?v=20260925-follow" />
    <link rel="stylesheet" href="/card-deck.css?v=20260926" /><script src="/card-deck.js?v=20260926" defer></script>
    ${profile.reviewKind === "connections" ? '<link rel="stylesheet" href="/company-directory.css" /><script src="/company-directory.js" defer></script>\n    ' : ""}<script src="/share-actions.js" defer></script>
    <script src="/video-viewer.js" defer></script>
  </head>
  <body class="profile-page${profile.reviewHtml || profile.companyCheckHtml ? " evidence-profile" : ""}${profile.reviewKind === "connections" ? " connections-profile" : ""}">
      ...
      <header class="hero article-hero profile-share-hero" id="top">
        <div class="hero-copy">
          <nav class="page-nav" aria-label="Навигация">
            <a href="${profile.reviewHtml || profile.companyCheckHtml ? "/#directory" : "/gallery.html#directory"}">← ${profile.reviewHtml || profile.companyCheckHtml ? "Всички профили" : "Архивни профили"}</a>
          </nav>
          <p class="eyebrow">${profile.reviewKind === "connections" ? "Фирмен справочник • Публични източници" : profile.reviewHtml ? "Разследване • Публични източници" : profile.companyCheckHtml ? "Фирмена проверка • Публични източници" : "Архив • Редакционен коментар"}</p>
          <h1>${escapeHtml(profile.name)}</h1>
          <p class="hero-text">${escapeHtml(profile.summary || profile.description || profile.imageNote)}</p>
          <p class="context-note">
            ${profile.reviewKind === "connections" ? "Собственост, управление и делови роли — с дата и източник." : profile.reviewHtml ? "Оферта, обещания и условия — проверени по посочените източници." : "Преглед на фирмената идентификация и публичните делови роли. Източниците и обхватът са посочени по-долу."} <a href="/contactus.html">Изпрати корекция или източник.</a>
          </p>
          <div class="hero-actions">
            ${reportHelpLink}
            <a class="button button-secondary" href="/scams.html">Разпознай тактиките</a>
          </div>
        </div>

        <div class="hero-visual">
          <div class="hero-frame">
            <img src="${escapeHtml(imagePath)}" alt="${escapeHtml(profile.alt)}" decoding="async" fetchpriority="high" />
          </div>
          <div class="hero-quote">
            <span>${escapeHtml(profile.kicker || "За какво да внимаваме")}</span>
            <strong>${escapeHtml(profile.imageNote || pageDescription)}</strong>
          </div>
        </div>
      </header>

      <main class="profile-share-main" id="main">
        ${profile.reviewHtml || profile.companyCheckHtml ? `<div class="evidence-page">${profile.companyCheckHtml || ""}${profile.reviewHtml || ""}<div class="share-button-group" aria-label="Опции за споделяне">${shareButtons.join("")}</div></div>` : `<article class="profile-card" id="${escapeHtml(profile.id)}" data-orientation="${escapeHtml(profile.orientation)}">
          <div class="profile-rail">
            <div class="profile-media">
              <img src="${escapeHtml(imagePath)}" alt="${escapeHtml(profile.alt)}" loading="lazy" decoding="async" fetchpriority="low" />
...
`;
  return profile.reviewHtml ? page.replace(/[ \t]+$/gm, "") : page;
}
...
export async function readProfiles(rootDir = process.cwd()) {
  ...
  const companyDirectory = JSON.parse(await fs.readFile(path.join(assetsDir, "Фирмени връзки", "review.json"), "utf8"));
  const companyChecks = new Map(companyDirectory.profileChecks.map(check => [check.profile, check]));
  const profiles = [];
  ...
  for (const folder of folders) {
    ...
    const { description, kicker, summary, aura, funnel, insight, imageNote, links, channels } =
      parseLinks(rawText);
    let reviewFields = {};
    if (files.some((entry) => entry.name === "review.json")) {
      const review = JSON.parse(await fs.readFile(path.join(folderPath, "review.json"), "utf8"));
      reviewFields = { reviewHtml: renderReview(review), ...(review.kind ? { reviewKind: review.kind } : {}), reviewedOn: review.reviewedOn, subjectType: review.subjectType,
        shareTitle: typeof review.shareTitle === "string" ? review.shareTitle.trim() : "",
        shareLogo: review.shareLogo === true,
        imageAlt: typeof review.imageAlt === "string" ? review.imageAlt.trim() : "" };
    }
    const companyCheck = companyChecks.get(profileId);
    if (profileId !== "firmeni-vrazki" && !companyCheck) throw new Error(`Missing company check: ${profileId}`);
    ...
    profiles.push({
      ...reviewFields,
      ...(companyCheck ? { companyCheckHtml: renderProfileCompanyCheck(companyCheck, companyDirectory), companyReviewedOn: companyCheck.reviewedOn } : {}),
      id: profileId,
      name: displayName,
      image: path.posix.join("assets", folder.name, imageFile.name),
      alt: reviewFields.imageAlt || (reviewFields.subjectType === "Organization" ? `Редакционна карта за ${displayName}` : `Профилно изображение за ${displayName}`),
      imageNote: companyCheck && !reviewFields.reviewHtml ? companyCheck.title : defaultImageNote,
      videos: videoFiles.map((entry) => path.posix.join("assets", folder.name, entry.name)),
      orientation,
      description: companyCheck && !reviewFields.reviewHtml ? companyCheck.finding : description,
      kicker: companyCheck && !reviewFields.reviewHtml ? "Фирмена проверка • Публични източници" : kicker,
      summary: companyCheck && !reviewFields.reviewHtml ? companyCheck.title : summary,
      aura,
      funnel,
      insight,
      channels,
...
export async function writeSiteData(rootDir = process.cwd()) {
  ...
  const thumbnailAssetsDir = path.join(staticAssetsDir, "thumbs");
  ...
  await fs.mkdir(thumbnailAssetsDir, { recursive: true });
  ...
  await generateShareCard(
    {
      guide: true,
      name: "Провери обещанията.\nПреди да платиш.",
      kicker: "РЕГИСТЪР И ПРАКТИЧНИ СЪВЕТИ",
      summary: "Оферти • Източници • Сигнали за риск",
    },
    path.join(rootDir, "scripts/render-share-card.swift"),
    path.join(staticAssetsDir, siteShareCardFileName),
  );
  for (const guide of [
    { file: "checklist-share-v2.jpg", name: "Преди да платиш:\n5 въпроса.", kicker: "ПРОВЕРИ ОФЕРТАТА", summary: "Цена. Съдържание. Доказателства.\nПродавач. Условия за отказ." },
    { file: "schemes-share-v2.jpg", name: "Откъде идват\nпарите?", kicker: "КАК РАБОТЯТ СХЕМИТЕ", summary: "Понци • Пирамиди • Онлайн курсове" },
  ]) {
    await generateShareCard({ ...guide, guide: true }, path.join(rootDir, "scripts/render-share-card.swift"), path.join(staticAssetsDir, guide.file));
  }
  ...
  for (const profile of profiles) {
    ...
    const thumbnailFileName = `${fileSlug}-thumb.jpg`;
    // New editorial copy gets a new URL so social previews do not reuse old images.
    const shareVersion = profile.shareTitle
      ? `-${createHash("sha256").update(JSON.stringify([profile.shareTitle, profile.kicker, profile.summary, ...(profile.shareLogo ? ["logo"] : [])])).digest("hex").slice(0, 8)}`
      : "";
    const shareCardFileName = `${fileSlug}-share${shareVersion}.jpg`;
    const sourceImagePath = path.join(rootDir, profile.image);
    const targetImagePath = path.join(staticAssetsDir, targetFileName);
    const thumbnailImagePath = path.join(thumbnailAssetsDir, thumbnailFileName);
    const shareCardPath = path.join(staticAssetsDir, shareCardFileName);

    await optimiseImageAsset(sourceImagePath, targetImagePath);
    await optimiseImageAsset(sourceImagePath, thumbnailImagePath, {
      maxDimension: thumbnailImageMaxDimension,
    ...
    for (const [index, videoPath] of (profile.videos || []).entries()) {
      ...
      await copyAssetIfNeeded(sourceVideoPath, targetVideoPath);
    ...
    staticProfiles.push({
      ...
      thumbnailImage: path.posix.join("site-assets", "thumbs", thumbnailFileName),
  ...
  const fileContent = `window.__GURU_PROFILES__=${JSON.stringify(staticProfiles)};\n`;

  await fs.writeFile(outputFile, fileContent, "utf8");
  await fs.writeFile(sitemapFile, buildSitemapXml(staticProfiles, siteBaseUrl), "utf8");
  await fs.writeFile(robotsFile, buildRobotsTxt(siteBaseUrl), "utf8");
  await fs.writeFile(noJekyllFile, "", "utf8");

  await writeDirectories(staticProfiles, rootDir);
