...
function getDefaultImageNote() {
  return "Изображение към профила.";
}
    ...
    <div class="page-shell">
      <header class="hero article-hero profile-share-hero" id="top">
        <div class="hero-copy">
          ...
          <p class="eyebrow">${profile.reviewKind === "connections" ? "Фирмен справочник • Публични източници" : profile.reviewHtml ? "Разследване • Публични източници" : profile.companyCheckHtml ? "Фирмена проверка • Публични източници" : "Архив • Публично представяне"}</p>
...
export async function writeSiteData(rootDir = process.cwd()) {
  ...
  for (const profile of profiles) {
    ...
    const shareVersion = `-${createHash("sha256").update(JSON.stringify([profile.shareTitle || profile.name, profile.kicker, profile.summary, ...(profile.shareLogo ? ["logo"] : [])])).digest("hex").slice(0, 8)}`;
