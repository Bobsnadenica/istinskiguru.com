...
async function generateShareCard(profile, sourceImagePath, outputImagePath) {
  if (await isOutputFresh(sourceImagePath, outputImagePath) &&
      (!profile.guide || await isOutputFresh(path.join(process.cwd(), "lib/profiles.mjs"), outputImagePath))) {
    return;
  }
...
<html lang="bg">
  <head>
    ...
    <meta name="twitter:image:alt" content="${escapeHtml(profile.alt || profile.name)}" />
