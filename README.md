...
The site helps Bulgarian readers understand Ponzi schemes, pyramid schemes, and online course sales. Keep these models distinct: an expensive course, a sales funnel, or inclusion in this directory does not establish fraud.

- `index.html`: short introduction, three model summaries, searchable profiles.
- `scams.html`: definitions, a worked Ponzi example, recruiting growth, and the course sales funnel. Links to SEC/Investor.gov, FTC, and Bulgarian FSC guidance are included beside the relevant content.
- `saveti.html`: five questions before paying, optional topic details, and reporting guidance.
- Profile pages and the gallery identify existing commentary as editorial/satirical. Public channel links are starting points for investigation, not independent corroboration.
- Reporting links lead to guidance and official channels; they do not generate allegations on a visitor's behalf.
...
TopCoding, Ivelin Mihaylov, Tsvetan Radushev, and „Как да издадеш книга в България“ (izdai-kniga.com) have dated, sourced reviews using the format below. The remaining 42 legacy profiles have not all undergone that evidence review. New reviews should use this standard, and existing profiles should be checked against it as they are updated.
...
Preview the homepage, guides, gallery, and a generated profile at desktop and phone widths. Check search (including no results), profile opening/closing and keyboard focus, video opening, guide anchors, and expandable advice topics. The educational guides work without JavaScript. Local browser artifacts belong in ignored `output/` and `.playwright-cli/` folders.
